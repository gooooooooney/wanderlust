...
export const renderBlocks = (blocks: BlockObjectResponse[], nestedLevel = 0) => {
  ...
  const getBulletedList = (block: BlockObjectResponse, index: number) => {
    ...
    if (currentList) {
      currentList.push(<BlockRenderer nestedLevel={nestedLevel} key={block.id} block={block} />);
    } else {
      currentList = [<BlockRenderer nestedLevel={nestedLevel} key={block.id} block={block} />];
    }
    const nextBlock = blocks[index + 1];
    const nextParent = getParentPage(nextBlock?.parent);
    const parent = getParentPage(block.parent);
    
    if (!(nextBlock && nextBlock.type === BLOCK_TYPES.BULLETED_LIST_ITEM && nextParent?.page_id === parent?.page_id)) {
      renderedElements.push(<ul className={cn(listClasses[cycle(nestedLevel)])}>{currentList}</ul>);
      currentList = null;
    }
  }
  const getNumberedList = (block: BlockObjectResponse, index: number) => {
    ...
    const nextParent = getParentPage(nextBlock?.parent);
    const parent = getParentPage(block.parent);
    if (!(nextBlock && nextBlock.type === BLOCK_TYPES.NUMBERED_LIST_ITEM && nextParent?.page_id === parent?.page_id)) {
      renderedElements.push(<ol className={cn(listClasses[cycle(nestedLevel)])}>{currentList}</ol>);
      currentList = null;
    }
  }
